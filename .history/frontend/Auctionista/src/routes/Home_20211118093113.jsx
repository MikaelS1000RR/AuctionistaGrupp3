...
const Home = () => {
  const { setBidTitle, setBidLocation, setBidCategory, setBidOnSell } =
    useBidContext();
  const { isLoggedIn } = useGlobal();
  const {getProducts, setProductsBySearch, notFound,
  ...
  const { saveSelectedLocation, saveSelectedCategory, saveInputedProduct } =
    useSearchParm();
    ...
    <div className="home">
      ...
      <div className="informationwrap">
        ...
        {!isLoggedIn && (
          <p className="information">
            ...
            You set starting price and duration of item on sale. Keep easy track
            of your auctions and biddings and search and filter through a
            variety of categories and products all ready to be bought.
          </p>
        )}
        {!isLoggedIn && (
          <button className="register">
            <Link to="/register" className="link">
              <img src={UploadIcon} /> Join now
            </Link>
          </button>
        )}
      </div>
      ...
      <div className="searchwrap">
        ...
        <div className="search">
          <button className="searchbtn" onClick={listProducts}>
            Search
          </button>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="upperproducts">
            ...
            {productsBySearch.length > 0 && (
              <p className="more" onClick={clear}>
                Clear
              </p>
            )}
          </div>
                    ...
                    >
                      <div className="productimg">
                        <img
                          ...
                          src={
                            product.imageUrl && product.imageUrl.split(",")[0]
                          }
                        />
                      </div>
                  ...
                  {!product.owner &&
                    ...
                    !product.isUserHighestBidder && (
                      <Bid
                        ...
                        maxBid={product.highestBid}
                      />
                    )}
                  {product.owner && !product.expired && (
                    <div className="bidbtn-wrap">
                      <button className="placebid">
                        <img src={UploadIcon} />
                        <p className="bidbtn-text">
                          You can not bid on your product
                        </p>
                      </button>
                    </div>
                  )}
