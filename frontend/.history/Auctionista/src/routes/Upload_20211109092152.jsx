...
const Upload = () => {
  ...
  const newSubmit = async (e) => {
    ...
    for(let title of data.keys()) {
      const input = form.elements[title];
      const parserTitle = input.dataset.parse
    

    if (parserTitle) {
      const parser = inputParsers[parserTitle];
      const parsedValue = parser(data.get(title));
      data.set(title, parsedValue);
    }
    }
